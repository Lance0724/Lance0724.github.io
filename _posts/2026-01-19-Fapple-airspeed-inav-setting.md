...
# 设置空速来源，AUTO 模式在传感器失效时会自动切换回虚拟空速
set airspeed_source = AUTO
# v9 新增：优化低通滤波，减少读数抖动（单位为毫赫兹）
...
1. **设置参考空速：** `set fw_reference_airspeed = 15` (设为你平时巡航的空速，单位 m/s)。
...
* **读数为负数：** 说明皮托管的动压管和静压管接反了，把传感器上的两根硅胶管对调即可。
...
# 3. 设置衰减源为自动/空速计
# v9 版本中，系统会自动检测。如果检测到空速计，会优先使用空速
set fw_pid_attenuation_source = AIRSPEED

# 4. (可选) v9 新增：俯仰角感知的 TPA 增强
set fw_pitot_pitch_aware_tpa = ON
...
* **如何确定：** 如果你是在 50% 油门、时速 54km/h () 下做的 Autotune，那么就设为 `15`。
